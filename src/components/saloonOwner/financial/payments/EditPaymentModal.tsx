...
export default function EditPaymentModal({ isOpen, onClose, onSave, initialData }: EditPaymentModalProps) {
  ...
  useEffect(() => {
    if (isOpen && initialData) {
      setFormData({
        ...
        teamMemberName: initialData.teamMember.name,
        teamMemberEmail: initialData.teamMember.email,
  ...
  const handleSubmit = () => {
    onSave({
      ...
      teamMember: { name: formData.teamMemberName, email: formData.teamMemberEmail },
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Payment" maxWidth="max-w-2xl">
