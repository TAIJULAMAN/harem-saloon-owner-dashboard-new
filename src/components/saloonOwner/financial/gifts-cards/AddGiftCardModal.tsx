...
export default function AddGiftCardModal({ isOpen, onClose, onSave }: AddGiftCardModalProps) {
  const [formData, setFormData] = useState({
    issueDate: new Date().toLocaleString("en-GB", { day: "numeric", month: "short", year: "numeric" }),
    expirationDate: "",
    amount: "",
  ...
  const handleSubmit = () => {
    onSave({
      id: "#" + Math.floor(Math.random() * 1000).toString().padStart(3, '0') + "B",
      issueDate: formData.issueDate,
      expirationDate: formData.expirationDate || "TBD",
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add Gift Card" maxWidth="max-w-2xl">
