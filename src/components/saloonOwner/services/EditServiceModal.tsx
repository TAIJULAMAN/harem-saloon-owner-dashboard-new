...
export default function EditServiceModal({ isOpen, onClose, onSave, initialData }: EditServiceModalProps) {
  ...
  useEffect(() => {
    if (isOpen && initialData) {
      ...
      setPostBreak(initialData.postBreak);
      setPrice(initialData.price);
      setVat(initialData.vat);
      setSelectedIcon(initialData.iconIndex || 0);
    }
  }, [isOpen, initialData]);

  const handleSave = () => {
    onSave({ ...initialData, name, category, duration, postBreak, price, vat, iconIndex: selectedIcon });
    onClose();
  };

  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Service" maxWidth="max-w-xl">
