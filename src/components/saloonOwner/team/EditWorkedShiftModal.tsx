...
interface EditWorkedShiftModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (date: string, timeIn: string, timeOut: string) => void;
  initialData?: { date: string; timeIn: string; timeOut: string };
}

export default function EditWorkedShiftModal({ isOpen, onClose, onSave, initialData }: EditWorkedShiftModalProps) {
  ...
  useEffect(() => {
    if (isOpen && initialData) {
      // Parse "Dec 01, 2024" to "YYYY-MM-DD" approximately for the native input, 
      // but for mockup purposes we can just set it and let the user see it, or use text type.
      // We'll use text type for date to make it easy to match the mockup exactly.
